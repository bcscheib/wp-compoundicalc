<?
	...
	$class_names = array(
		'pages' => apply_filters( 'wp_compoundicalc_class_pages', 'pages'),
		'first' => apply_filters( 'wp_compoundicalc_class_first', 'first' ),
		'previouspostslink' => apply_filters( 'wp_compoundicalc_class_previouspostslink', 'previouspostslink' ),
		'extend' => apply_filters( 'wp_compoundicalc_class_extend', 'extend' ),
		'smaller' => apply_filters( 'wp_compoundicalc_class_smaller', 'smaller' ),
		'page' => apply_filters( 'wp_compoundicalc_class_page', 'page' ),
		'current' => apply_filters( 'wp_compoundicalc_class_current', 'current'),
		'larger' => apply_filters( 'wp_compoundicalc_class_larger', 'larger' ),
		'nextpostslink' => apply_filters( 'wp_compoundicalc_class_nextpostslink', 'nextpostslink'),
		'last' => apply_filters( 'wp_compoundicalc_class_last', 'last'),
	);
		...
		case 1:
			...
			if ( !empty( $options['pages_text'] ) ) {
				$pages_text = str_replace(
					...
					__( $options['pages_text'], 'wp-compoundicalc' ) );
				$out .= "<span class='{$class_names['pages']}'>$pages_text</span>";
			}

			if ( $start_page >= 2 && $pages_to_show < $total_pages ) {
				// First
				$first_text = str_replace( '%TOTAL_PAGES%', number_format_i18n( $total_pages ), __( $options['first_text'], 'wp-compoundicalc' ) );
			...
			foreach ( $larger_pages_array as $larger_page ) {
				if ( $larger_page < ($start_page - $half_page_start) && $larger_page_start < $larger_page_to_show ) {
					$out .= $instance->get_single( $larger_page, $options['page_text'], array(
						...
						'title' => sprintf( __( 'Page %d', 'wp-compoundicalc' ), number_format_i18n( $larger_page ) ),
					) );
					$larger_page_start++;
				}
			}
			...
			foreach ( range( $start_page, $end_page ) as $i ) {
				if ( $i == $paged && !empty( $options['current_text'] ) ) {
					...
					$out .= $instance->get_single( $i, $options['page_text'], array(
						...
						'title' => sprintf( __( 'Page %d', 'wp-compoundicalc' ), number_format_i18n( $i ) ),
					) );
				}
			}
			...
			if ( $end_page < $total_pages ) {
				// Last
				$out .= $instance->get_single( $total_pages, __( $options['last_text'], 'wp-compoundicalc' ), array(
